...
export const constructSlackMessage = async (
  ...
  let prDescription = `${
    ...
    author.slackmoji ? ` ${author.slackmoji}` : ""
  }`;
  ...
  const prReviews = await github_adapter.platform_methods.getPrReviews(
    pullyRepodataCache,
    prNumber,
  );

  const reviewRequests = await github_adapter.platform_methods
    .getReviewsRequestedForPr(pullyRepodataCache, prNumber);
  ...
  for (const review of prReviews) {
    if (review.author?.githubUsername !== undefined) {
      ...
      if (
        !existing ||
        (reviewStatePriority(review.state) >=
            reviewStatePriority(existing.state) &&
  ...
  for (const [reviewer, state] of Object.entries(reviews)) {
    ...
    switch (state.state) {
      case "approved":
        approvers.add(
          `${reviewerData.firstName ?? reviewerData.githubUsername} ${
            reviewerData.slackmoji ? `${reviewerData.slackmoji}` : ""
          }`,
        );
        break;
      ...
      case "review_requested":
        ...
        review_requests.add(
          reviewerData.slackMemberId
            ? `<@${reviewerData.slackMemberId}>`
            : `${reviewerData.githubUsername}`,
        );
    }
  }

  const prIsClosed = prState === "closed" || prState === "merged";
  const hideReviews = prIsClosed &&
    pully_options.PULLY_HIDE_REVIEWS_WHEN_PR_CLOSED;
  ...
  let leftHandSideText = `${
    generateSlackLink(prUrl, repoNameFormatted)
  } ${prDescription}`;
  ...
  const slackMessage =
    reviewStatusText && pully_options.PULLY_REVIEW_STATUS_ON_NEW_LINE
