...
Deno.test("commenter is shown with speech bubble when reviewer only comments", async () => {
      ...
      getPrReviews: async () => [
        {
          author: reviewer,
          time: new Date(),
          state: "commented" as ReviewerState,
        },
      ],
    },
  };
  ...
  assertEquals(
    result,
    ":github-pr: <https://github.com/owner/repo/pull/5|[owner/repo]> My PR (#5)  by Alice | :speech_balloon: Bob ",
  );
});
...
Deno.test("approval takes precedence over comment when reviewer comments then approves", async () => {
  ...
  assertEquals(
    result,
    ":github-pr: <https://github.com/owner/repo/pull/6|[owner/repo]> My PR (#6)  by Alice | :github-approve: Bob ",
  );
});
...
Deno.test("approval takes precedence over comment when reviewer approves then comments", async () => {
  ...
  assertEquals(
    result,
    ":github-pr: <https://github.com/owner/repo/pull/7|[owner/repo]> My PR (#7)  by Alice | :github-approve: Bob ",
  );
});
...
Deno.test("hides review line on closed PR when PULLY_HIDE_REVIEWS_WHEN_PR_CLOSED is set", async () => {
  ...
  assertEquals(
    result,
    ":github-closed: ~<https://github.com/owner/repo/pull/9|[owner/repo]> My PR (#9)  by Alice~",
  );
});
...
Deno.test("hides review line on merged PR when PULLY_HIDE_REVIEWS_WHEN_PR_CLOSED is set", async () => {
  ...
  assertEquals(
    result,
    ":github-merged: ~<https://github.com/owner/repo/pull/10|[owner/repo]> My PR (#10)  by Alice~",
  );
});
...
Deno.test("still shows review line on open PR when PULLY_HIDE_REVIEWS_WHEN_PR_CLOSED is set", async () => {
  ...
  assertEquals(
    result,
    ":github-pr: <https://github.com/owner/repo/pull/11|[owner/repo]> My PR (#11)  by Alice | :github-approve: Bob ",
  );
});
...
Deno.test("review status is on a second line when PULLY_REVIEW_STATUS_ON_NEW_LINE is set", async () => {
  ...
  assertEquals(
    result,
    ":github-pr: <https://github.com/owner/repo/pull/8|[owner/repo]> My PR (#8)  by Alice\n:github-approve: Bob ",
  );
});
