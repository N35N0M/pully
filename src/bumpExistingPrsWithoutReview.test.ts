...
import { bumpExistingPrsWithoutReview, REMINDER_MESSAGES } from "./bumpExistingPrsWithoutReview.ts";
...
Deno.test("skips PR when it has an approval", async () => {
	...
	await bumpExistingPrsWithoutReview(pullyData, makeAdapter({
		getPrReviews: async () => [{ author: {}, time: new Date(), state: "approved" as ReviewerState }],
...
Deno.test("skips PR when it has a change request", async () => {
	let posted = false;
	await bumpExistingPrsWithoutReview(pullyData, makeAdapter({
		getPrReviews: async () => [{ author: {}, time: new Date(), state: "requested-changes" as ReviewerState }],
	}), pullyOptions, async () => { posted = true; return undefined; });
...
Deno.test("skips PR when 5 reminders already sent", async () => {
	let posted = false;
	await bumpExistingPrsWithoutReview(pullyData, makeAdapter({
		getReminderTimestampsForPr: async () => [yesterdayTs, yesterdayTs, yesterdayTs, yesterdayTs, yesterdayTs],
	}), pullyOptions, async () => { posted = true; return undefined; });
...
Deno.test("sends first reminder when no prior reminders", async () => {
	let sentMessage = "";
	await bumpExistingPrsWithoutReview(pullyData, makeAdapter(), pullyOptions,
		async (message) => { sentMessage = message; return "reply.ts"; });
...
Deno.test("sends correct unique message based on prior reminder count", async () => {
	let sentMessage = "";
	await bumpExistingPrsWithoutReview(pullyData, makeAdapter({
		getReminderTimestampsForPr: async () => [yesterdayTs, yesterdayTs, yesterdayTs],
	}), pullyOptions, async (message) => { sentMessage = message; return "reply.ts"; });
...
Deno.test("sends fifth (final) reminder after four prior", async () => {
	let sentMessage = "";
	await bumpExistingPrsWithoutReview(pullyData, makeAdapter({
		getReminderTimestampsForPr: async () => [yesterdayTs, yesterdayTs, yesterdayTs, yesterdayTs],
	}), pullyOptions, async (message) => { sentMessage = message; return "reply.ts"; });
...
Deno.test("includes slack mention when review was requested", async () => {
	let sentMessage = "";
	await bumpExistingPrsWithoutReview(pullyData, makeAdapter({
		getReviewsRequestedForPr: async () => [{ slackMemberId: "U123", githubUsername: "bob" }],
	}), pullyOptions, async (message) => { sentMessage = message; return "reply.ts"; });
...
Deno.test("stores the reply timestamp after sending", async () => {
	let storedTs = "";
	await bumpExistingPrsWithoutReview(pullyData, makeAdapter({
		addReminderTimestampForPr: async (_prNumber, ts) => { storedTs = ts; },
	}), pullyOptions, async () => "stored.ts");
